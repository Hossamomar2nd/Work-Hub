    ...
    if (adminToUpdate) {
      if (adminData.includes(req.body.password)) {
        ...
        if (match) {
          ...
          if (!newMatch) {
            if (newPassword === confirmNewPassword) {
              ...
              if (validatePassword(newPassword)) {
                ...
                const update = {
                  $set: { Password: newPasswordHash, token: "null" },
                };

                await AdminModel.updateOne(filter, update);
                return res
                  .status(200)
                  .send("Admin has been updated successfuly.");
              }
              return res
                .status(400)
                .send(
                  "Password is not valid. Please follow the password pattern.",
                );
            }
          ...
          return res
            .status(400)
            .send("You cannot use your current password as new password.");
        }
...
export const deleteAdmin = async (req, res) => {
  try {
    ...
    if (adminToDelete) {
      ...
      return res
        .status(200)
        .json({ msg: "Admin has been deleted successfuly." });
    }
